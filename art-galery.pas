...
begin 
    ...
    begin
        Line(Round(room[1][i]), Round(room[2][i]), Round(room[1][i + 1]), Round(room[2][i + 1]));
        Circle(Round(room[1][i]), Round(room[2][i]), 10);
        TextOut(Round(room[1][i]), Round(room[2][i]), i);
        Sleep(2000);
    end;    
...
var
    num_add: int64;//num_points : длина массива intersections, num_add : длина массива add
begin
...
var
    counter, start, direction, num_vert1, num_vert2: int64;
var
    intersections1, intersections2: booleans;
var
    in_fig1: boolean;
var
    fig1, fig2: figure;
begin
    fig1 := figure1; // здесь будут все точки фигуры 1
    fig2 := figure2;// здесь будут все точки фигуры 2
    num_vert1 := num_vertex1; // сколько вершин в первой фигуре
    num_vert2 := num_vertex2; // сколько во второй
    // вставляет все точки пересечения в фигуру
    insert_intersections(fig1, fig2, num_vert1, num_vert2, intersections1, intersections2);
    Print(num_vert1, num_vert2);
    //ОБЪЕДИНЕНИЕ
    if num_vertex1 = num_vert1 then
    begin 
        result := false;
        exit;
    end;
